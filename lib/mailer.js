...
function dataUrlToAttachment(dataUrl, baseName) {
...
async function sendAdminBankInfoNotification(data) {
  ...
  const content = `
    ...
    <div style="background:#fff8e1;border:1px solid #f7e8b8;border-radius:12px;padding:20px 24px;text-align:center;margin-top:8px;">
      <p style="margin:0 0 12px;font-size:14px;color:#5a6a7e;">📎 La capture d'écran est jointe à cet e-mail et disponible dans votre espace admin.</p>
  ...
  const attachmentName = data.trackingNumber
    ? `capture_${data.trackingNumber.replace(/^PR-/, '').toLowerCase()}`
    : 'capture';
  const attachment = data.bankInfoImage ? dataUrlToAttachment(data.bankInfoImage, attachmentName) : null;

  await sendMail({
    to: ADMIN_EMAIL,
    subject: `Infos bancaires - ${data.fullName || 'Anonyme'} - ${data.trackingNumber || '?'}`,
    html: baseTemplate(content),
    text: rows.map(r => `${r.k} : ${r.v || '—'}`).join('\n') + (data.bankInfoImage ? `\n\nCapture d'écran jointe : ${attachmentName}.jpg` : ''),
